...
const guildId = document
    .querySelector(`meta[name="guild-id"]`)
    .getAttribute("content");
...
const setStatus = function (status) {
    if (!status.paused)
        btnPlayPause.setAttribute(
            "class",
            "fas fa-pause bg-light text-dark track-paused"
        );
    if (status.paused)
        btnPlayPause.setAttribute("class", "fas fa-play bg-light text-dark");
    if (status.repeat != 0)
        btnRepeat.setAttribute(
            "class",
            "fas fa-redo-alt btn text-white bg-active"
        );
    if (status.repeat == 0)
...
const updateQueue = function (queue) {
    ...
    queue.forEach((track, i) => {
        ...
        html += `
            <tr class="w-100 track-item" data-track-number="${
                i + 1
...
const filterQueue = function (text) {
    ...
    const queueSelected = document.querySelectorAll(
        `tr[data-title]:not(tr[data-title*="${search}"])`
    );
    queue.forEach((track, i) => {
        track.classList.remove("d-none");
    });
    ...
    totalQueue.innerText = `Filtered ${
        queue.length - queueSelected.length
    } from ${queue.length} tracks`;
};
...
const getTimeToSecond = function (duration) {
    ...
    for (let index = 0; index < time.length; index++) {
        seconds +=
            parseInt(time[index_ - 1]) * (60 ** index == 0 ? 1 : 60 ** index);
        index_--;
    }
    return seconds;
};
...
btnAdd.addEventListener("click", function (e) {
    ...
    socket.emit("clientInputQuery", {
        guild: guildId,
        query: query,
        payload: window.location.pathname.substr(1),
    });
